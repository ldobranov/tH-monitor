...
try:
    display = drivers.Lcd()
    logging.warning(datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ') + "LCD started")
except:
    logging.error(datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ') + "LCD error")
    pass
...
while True:
    try:
        ...
        if (result1.get('valid') == True):
            ...
            logging.info(datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ') + "sensor 1 readed")
    except:
        logging.debug(datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ') + " Sensor 1 error")
        pass
    ...
    try:
        ...
        if (result2.get('valid') == True):
            ...
            logging.info(datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ') + "Sensor 2 readed")
    except:
        logging.debug(datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ') + " Sensor 2 error")
        pass
    ...
    logging.info(datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ') + "Start saving to db...")
    ...
    try:
        ...
        logging.info(current_time.strftime('%Y-%m-%dT%H:%M:%SZ') + "  Saved to db")

    #finally:
    #db.close()
    except:
        logging.error(current_time.strftime('%Y-%m-%dT%H:%M:%SZ') + "  ERROR Saving to db")
        pass
